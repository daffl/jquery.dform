...
[];a.each(b,function(f){d.push(f)});return d},withKeys:function(b,d){var f={};a.each(d,function(g,i){if(b[i])f[i]=b[i]});return f},withoutKeys:function(b,d){var f={};a.each(b,function(g,i){if(a.inArray(g,d)==-1)f[g]=i});return f},getValueAt:function(b,d){for(var f=a.isArray(d)?d:d.split("."),g=b,i=0;i<f.length;i++){var j=f[i];if(!g[j])return false;g=g[j]}return g}});a.dform={options:{prefix:"ui-dform-",ajaxFormat:"json",defaultType:function(b){return a("<"+b.type+">").dformAttr(b)}},removeType:function(b){delete e[b]},
typeNames:function(){return a.keyset(e)},addType:function(b,d){h(e,b,d)},addTypeIf:function(b,d,f){b&&a.dform.addType(d,f)},subscriberNames:function(){return a.keyset(c)},subscribe:function(b,d){h(c,b,d)},subscribeIf:function(b,d,f){b&&a.dform.subscribe(d,f)},removeSubscription:function(b){delete c[b]},hasSubscription:function(b){return c[b]?true:false},createElement:function(b){var d=b.type;if(!d)throw"No element type given! Must always exist.";var f=null;if(e[d]){var g=a.withoutKeys(b,"type");a.each(e[d],
function(i,j){f=j.call(f,g)})}else f=a.dform.options.defaultType(b);return a(f)}}})(jQuery);(function(a){a.fn.placeholder=function(h){var c=this;a(this).data("placeholder",h);a(this).val(h);a(this).focus(function(){a(this).val()==a(this).data("placeholder")&&a(this).val("")});a(this).blur(function(){a(this).val()==""&&a(this).val(a(this).data("placeholder"))});a(this).parents("form").submit(function(){a(c).val()==a(c).data("placeholder")&&a(c).val("")});return this}})(jQuery);
...
"string")g.caption=f;else a.extend(g,f);g.value=d;a(b).formElement(g)})}},caption:function(c,e){if(e=="fieldset"){var b={type:"legend"};if(typeof c=="string")b.html=c;else a.extend(b,c);var d=a.dform.createElement(b);this.prepend(d);a(d).runAll(b)}else{b={type:"label"};if(typeof c=="string")b.html=c;else a.extend(b,c);if(this.attr("id"))b["for"]=this.attr("id");d=a.dform.createElement(b);e=="checkbox"||e=="radio"?this.parent().append(a(d)):a(d).insertBefore(a(this));a(d).runAll(b)}},type:function(c,
e){a.dform.options.prefix&&this.addClass(a.dform.options.prefix+e)},"[post]":function(c,e){if(e=="checkboxes"||e=="radiobuttons")this.children("[type="+(e=="checkboxes"?"checkbox":"radio")+"]").each(function(){a(this).attr("name",c.name)})}})})(jQuery);
